...
$$
\begin{aligned}
\text{proportional increase in error}&= \frac{\text{change in error (from $M_0$ to $M_1$)}}{\text{minimal error}} ,\\
\\
...
$$
\begin{aligned}
\text{proportional decrease in error}&= \frac{\text{change in error (from $M_0$ to $M_1$)}}{\text{error in $M_0$}} ,\\
\\
...
-   The *deviance* of a model is defined
    $$-2 \log  \hat{\mathcal{L}}$$ where
    $\hat{\mathcal{L}}$ is the likelihood of the model using the mle estimates.
